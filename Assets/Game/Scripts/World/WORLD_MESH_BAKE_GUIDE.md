...
`WorldLand` prefab 에 **MeshCollider + Landmass 컴포넌트** 가 자동 부착됨 → 진짜 해안선 충돌이 작동.
따라서 기존 큐브 콜라이더는 불필요. `LandmassRoot` 선택 후:

| 체크박스 | 설명 |
|---|---|
| **Hide Visuals ☑** | 큐브 MeshRenderer 끔 (지도를 안 가림) |
| **Disable Collision ☑** | 큐브 BoxCollider 제거 (메쉬가 충돌 담당) |

**둘 다 켜야** 새 메쉬가 시각·충돌 모두 담당하는 깔끔한 상태가 됩니다.

세 가지 요소 역할 분담:
1. `SeaPlane` — 푸른 바다 텍스처 (y = -0.05) — 시각만
2. `WorldLand` — 갈색 대륙 메쉬 (y = 0.15 ~ 2.65) — **시각 + 충돌**
3. `LandmassRoot/*` — 큐브 — Hide + Disable ☑ 면 사실상 무력화 (LandmassData SO 만 참조용)

### Prefab 갱신 주의

이미 씬에 `WorldLand` 를 드래그해둔 상태에서 다시 베이크하면, MeshCollider 가 새로 추가됐어도 **씬 인스턴스가 자동 갱신 안 될 수 있음**.

해결: 씬의 `WorldLand` 선택 → 우클릭 → **Prefab ▸ Revert (모든 Override 제거)** 또는 인스턴스 삭제 후 Prefab 다시 드래그.
