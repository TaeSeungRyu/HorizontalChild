...
### §8.19 — 8개국 시작 시점의 시대 격차 처리 ✅ 옵션 B 1차 결정 (2026-05-29)

`CONTENT_DESIGN.md` §2.1 의 8개국 NationData는 시작 시점이 모두 다르다 (1394~17세기 초, 약 200년 격차).

**확정 — 옵션 B: 국적별 시작 시점 분리 (시대 격차 인정).**

- NationData 의 "시작 시점" 필드를 **그대로 유지.** 통일하지 않음.
- 게임 내 시간은 각 국적이 자기 시점에서 시작.
- 같은 항구에서 만나는 NPC·시세는 시점에 따라 달라질 수 있음 (실제 구현 시 정교화).
- "어린이 대항해시대" 라는 게임이지만 사용자 결정에 따라 사실적 시대 분리를 유지.

**다음 결정 시점**: M3 진입 시 — 시점 분리가 게임 진행에 미치는 영향을 본격 구체화. NationData 모델에 "활동 가능 게임 시간 범위(`activeFromYear`/`activeToYear`)" 필드 추가 검토. NPC 등장 조건이 게임 시점과 연동될지 결정.

### §8.20 — 발견물 좌표 거리의 항해 부담 ✅ 권장 채택 (2026-05-29)

**확정 — 권장 채택**: "단거리/중거리/장거리" 분류로 보상 차등. 절대 거리 제한이 아닌 **분류 가이드 + 보상 차등** 으로 적용.

#### 거리 분류 가이드

| 분류 | 거리 기준 | 게임 내 시간 (가장 빠른 배 기준) | 보상 가이드 (돈/명성) |
|---|---|---|---|
| 단거리 | < 500 km | ~7초 이내 | 500 / +50 |
| 중거리 | 500 ~ 2,000 km | 7~30초 | 1,500 / +100 |
| 장거리 | > 2,000 km | 30초~1분+ | 2,000~3,000 / +150~200 |

**이 가이드는 이미 `CONTENT_DESIGN.md` §2.7 의뢰 시드 18개에 적용되어 있음.** 신규 의뢰 추가 시 거리 분류를 보고 보상 결정.

세계일주 10분 기준 (§3.1) 으로 보면 2,000km 도 ~30초로 어린이 인내 한도 안. 따라서 한라산(320km)·카나리아(1,500km)·부산↔광저우(2,200km) 같은 의뢰는 그대로 유지 가능.

#### 거리 → 게임 시간 환산 (잠정)

- 지구 둘레 약 40,000km / 세계일주 10분 = **약 67 km/초** (캐러벨 속도 8 기준)
- 실제 구현 시 §8.9 의 위·경도 → Unity 월드 좌표 매핑 결정과 함께 정교화.
